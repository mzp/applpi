...
let outAtom : bool -> 'a1 channel -> 'a1 -> 'b =
  fun b x v -> outP (b, Obj.magic x, Obj.magic v, (fun _ -> zeroP))


(** val is_nullstr : string -> bool **)

let is_nullstr s = if String.length s = 0 then true else false
...
let is_not_null_a_d_l s = if String.length s = 0 then false else true
...
let succ = outAtom false result_chan ()

type mail =
  { domain : string; rev : string; fwd : string list; body : string }
...
type sMTPstate =
  { in_stream : sMTP_cmd channel;
    to_client : replyMsg channel;
    server_name : string;
    queue_dir : string;
    buf : string;
    from_domain : string;
    rev_path : string;
    fwd_paths : string list;
    to_fs : mail channel;
    quit : bool }
...
let update_from st dom =
  {in_stream = st.in_stream; to_client = st.to_client;
   server_name = st.server_name; queue_dir = st.queue_dir; buf = st.buf;
   from_domain = dom; rev_path = st.rev_path; fwd_paths = st.fwd_paths;
   to_fs = st.to_fs; quit = st.quit}
...
let update_rev_path st path =
  {in_stream = st.in_stream; to_client = st.to_client;
   server_name = st.server_name; queue_dir = st.queue_dir; buf = st.buf;
   from_domain = st.from_domain; rev_path = path; fwd_paths = st.fwd_paths;
   to_fs = st.to_fs; quit = st.quit}
...
let update_fwd_paths st paths =
  {in_stream = st.in_stream; to_client = st.to_client;
   server_name = st.server_name; queue_dir = st.queue_dir; buf = st.buf;
   from_domain = st.from_domain; rev_path = st.rev_path; fwd_paths = paths;
   to_fs = st.to_fs; quit = st.quit}

(** val fwd_paths_add : sMTPstate -> string -> sMTPstate **)

let fwd_paths_add st path = update_fwd_paths st (Cons (path, st.fwd_paths))

type sTATE = sMTPstate

(** val reply : replyMsg -> replyMsg channel -> proc -> proc **)

let reply r c cont = outP (true, Obj.magic c, Obj.magic r, (fun _ -> cont))

(** val get_cmd : sMTP_cmd channel -> (sMTP_cmd -> proc) -> proc **)

let get_cmd c cont = inP (true, Obj.magic c, Obj.magic cont)

(** val parseRfc821path : string -> (unit, string) eXCEPT **)

let parseRfc821path s = SUCC (s, ())

(** val save_mail : string -> sTATE -> proc -> proc **)

let save_mail data st cont =
  outP
    (true, Obj.magic st.to_fs,
     Obj.magic
       {domain = st.from_domain; rev = st.rev_path; fwd = st.fwd_paths;
        body = data},
     (fun _ -> cont))

(** val reply_unknown_cmd : replyMsg channel -> proc -> proc **)

let reply_unknown_cmd c cont = reply Rep_unknown_cmd c cont

(** val reply_ok_helo : replyMsg channel -> proc -> proc **)

let reply_ok_helo c cont = reply Rep_ok_helo c cont

(** val reply_ok_mail_from : replyMsg channel -> proc -> proc **)

let reply_ok_mail_from c cont = reply Rep_ok_mail_from c cont

(** val reply_bad_mail_from : string -> replyMsg channel -> proc -> proc **)

let reply_bad_mail_from s c cont = reply (Rep_bad_mail_from s) c cont

(** val reply_ok_rcpt_to : replyMsg channel -> proc -> proc **)

let reply_ok_rcpt_to c cont = reply Rep_ok_rcpt_to c cont

(** val reply_bad_rcpt_to501 : replyMsg channel -> proc -> proc **)

let reply_bad_rcpt_to501 c cont = reply Rep_bad_rcpt_to501 c cont

(** val reply_bad_rcpt_to553 : replyMsg channel -> proc -> proc **)

let reply_bad_rcpt_to553 c cont = reply Rep_bad_rcpt_to553 c cont

(** val reply_bad_helo : replyMsg channel -> proc -> proc **)

let reply_bad_helo c cont = reply Rep_bad_helo c cont

(** val reply_ok_quit : replyMsg channel -> proc -> proc **)

let reply_ok_quit c cont = reply Rep_ok_quit c cont

(** val reply_ok_rset : replyMsg channel -> proc -> proc **)

let reply_ok_rset c cont = reply Rep_ok_rset c cont

(** val reply_ok_noop : replyMsg channel -> proc -> proc **)

let reply_ok_noop c cont = reply Rep_ok_noop c cont

(** val reply_ok_data : replyMsg channel -> proc -> proc **)

let reply_ok_data c cont = reply Rep_ok_data c cont

(** val reply_no_helo : replyMsg channel -> proc -> proc **)

let reply_no_helo c cont = reply Rep_no_helo c cont

(** val reply_no_mail_from : replyMsg channel -> proc -> proc **)

let reply_no_mail_from c cont = reply Rep_no_mail_from c cont

(** val reply_no_rcpt_to : replyMsg channel -> proc -> proc **)

let reply_no_rcpt_to c cont = reply Rep_no_rcpt_to c cont

(** val reply_dup_helo : replyMsg channel -> proc -> proc **)

let reply_dup_helo c cont = reply Rep_dup_helo c cont

(** val reply_dup_mail_from : replyMsg channel -> proc -> proc **)

let reply_dup_mail_from c cont = reply Rep_dup_mail_from c cont
...
let do_rcpt_to arg st cont =
  match parseRfc821path arg with
    SUCC (p, u) ->
      begin match is_not_null_a_d_l p with
...
let get_helo_def heloc mailc st =
  get_cmd st.in_stream
    (fun c ->
       ...
       | Cmd_unknown ->
           reply_unknown_cmd st.to_client (outAtom true heloc st))
...
let get_mail_def mailc rcptc st =
  get_cmd st.in_stream
    (fun c ->
       ...
       | Cmd_abort -> reply_ok_quit st.to_client succ
       | Cmd_unknown ->
           reply_unknown_cmd st.to_client (outAtom true mailc st))
...
let get_rcpt_def mailc rcptc st =
  get_cmd st.in_stream
    (fun c ->
       ...
       | Cmd_data data ->
           begin match fwd_paths_size_gt_0 st.fwd_paths with
             true ->
               save_mail data st
       ...
       | Cmd_abort -> reply_ok_quit st.to_client succ
       | Cmd_unknown ->
           reply_unknown_cmd st.to_client (outAtom true rcptc st))
                 ...
                 parP
                   (rinP
                      (true, heloc,
                       Obj.magic
                         (fun x ->
                            get_helo_def (Obj.magic heloc) (Obj.magic mailc)
                              x)),
                    parP
                      (rinP
                         (true, mailc,
                       ...
                       parP
                         (rinP
                            (true, rcptc,
                             Obj.magic
                               (fun x ->
                                  get_rcpt_def (Obj.magic mailc)
                                    (Obj.magic rcptc) x)),
                          outAtom true (Obj.magic heloc)
                            {in_stream = c1; to_client = c2;
                             server_name = server_name0;
                             queue_dir = queue_dir0; buf = buf0;
                             from_domain = from_domain0; rev_path = rev_path0;
                             fwd_paths = Nil; to_fs = tofs;
                             quit = false}))))))
...
let client s1 s2 =
  parP
    (outP (true, s1, Cmd_quit, (fun _ -> zeroP)),
     inP (true, s2, (fun _ -> zeroP)))

let file_system tofs = inP (true, tofs, (fun s -> zeroP))
...
let main () =
  ...
  parP
    (work s1 s2 tofs,
     parP
       (client s1 s2,
        parP (file_system tofs, inP (true, result_chan, (fun _ -> zeroP)))))
